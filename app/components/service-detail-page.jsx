...
export function ServiceDetailPage({ slug, showcase, heroImage, faqHeading, faqIntro, hideBottomCta = false }) {
  ...
  return <>
    ...
    <section className="section shell faq-grid"><div><p className="eyebrow">Frequently asked questions</p><h2>{faqHeading || "Answers before you commit to a project."}</h2>{faqIntro && <p>{faqIntro}</p>}</div><div>{pageFaqs.map((faq) => <details key={faq.question}><summary>{faq.question}</summary><p>{faq.answer}</p></details>)}</div></section>
