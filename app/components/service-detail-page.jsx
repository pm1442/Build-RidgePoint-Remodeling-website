...
export function ServiceDetailPage({ slug, showcase, heroImage, faqHeading, faqIntro, faqCtaLabel, centeredScope = false, hideIntro = false, hideBottomCta = false }) {
  ...
  return <>
    ...
    <section className={`page-hero shell service-hero${heroImage ? " service-hero-with-image" : ""}`}><div className="service-hero-copy"><p className="eyebrow">Serving Lake Butler and North Florida</p><h1>{service.name}</h1><p>{service.summary}</p><div className="service-hero-actions"><Link className="button" href="/contact">Request a Quote</Link><Link className="button button-outline" href={projectGalleryLink.href}>{projectGalleryLink.label}</Link></div></div>{heroImage && <figure className="service-hero-media"><Image src={heroImage.src} alt={heroImage.alt} width={1024} height={576} sizes="(max-width: 820px) 100vw, 52vw" preload /></figure>}</section>
