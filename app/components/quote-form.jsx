...
export function QuoteForm() {
  ...
  return <form className="quote-form" onSubmit={handleSubmit}><label>Project type<select name="projectType" defaultValue=""><option value="" disabled>Select a service</option>{services.map((service) => <option key={service.slug}>{service.name}</option>)}</select></label><label>Your name<input name="name" autoComplete="name" required /></label><label>Email or phone<input name="contact" autoComplete="email" required /></label><label>Project details<textarea name="details" rows="5" required /></label><button className="button" type="submit">Request a Free Estimate</button><p className="form-note">This opens a pre-filled email to Jason. Send the email to submit your request. If your device does not use an email app, call {site.phoneDisplay} instead.</p>{sent && <p className="form-success">Your email draft is ready. Once you send it, RidgePoint will have the details needed to follow up about your project.</p>}</form>;
}
